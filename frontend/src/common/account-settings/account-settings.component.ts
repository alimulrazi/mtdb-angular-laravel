...
export class AccountSettingsComponent implements OnInit, AfterViewInit {
    @ViewChild('extraPanelRef', {read: ViewContainerRef})
    extraPanelRef: ViewContainerRef;
    ...
    ngOnInit() {
        this.route.data.subscribe(
            (data: {api: AccountSettingsResolverData}) => {
    ...
    public updateAccountSettings() {
        ...
        this.users
            .update(this.currentUser.get('id'), this.userForm.value)
            ...
            .subscribe(
                ...
                (errResponse: BackendErrorResponse) =>
                    this.userErrors$.next(errResponse.errors)
            );
    }
    ...
    public openAvatarUploadDialog() {
        ...
        openUploadWindow({types: [UploadInputTypes.image]}).then(files => {
            ...
            this.users
                .uploadAvatar(this.currentUser.get('id'), files)
                .pipe(finalize(() => this.loading$.next(false)))
                .subscribe(
                    response => {
    ...
    public deleteAvatar() {
        this.loading$.next(true);
        this.users
            .deleteAvatar(this.currentUser.get('id'))
            .pipe(finalize(() => this.loading$.next(false)))
            .subscribe(user => {
    ...
    public changeUserPassword() {
        this.loading$.next(true);
        this.users
            .changePassword(this.currentUser.get('id'), this.passwordForm.value)
            .pipe(finalize(() => this.loading$.next(false)))
            .subscribe(
                () => {
                ...
                (errResponse: BackendErrorResponse) =>
                    this.passwordErrors$.next(errResponse.errors)
            );
    }
    ...
    private loadExtraPanels() {
        ...
        this.extraPanels.forEach(
            (panelComp: {component: ComponentType<any>}) => {
                ...
                const componentRef =
                    this.extraPanelRef.createComponent(componentFactory);
    ...
    public apiEnabled(): boolean {
        return (
            this.settings.get('api.integrated') &&
            this.currentUser.hasPermission('api.access')
        );
    }
                ...
                if (confirmed) {
                    this.auth.revokeAccessToken(token.id).subscribe(() => {
                        this.initialUser$.next({
                            ...
                            tokens: this.initialUser$.value.tokens.filter(
                                t => t.id !== token.id
                            ),
                        });
    ...
    public openCreateTokenDialog() {
        this.dialog
            .open(CreateAccessTokenDialogComponent)
            .afterClosed()
            .subscribe(token => {
                if (token) {
                    this.initialUser$.next({
    ...
    openConfirmAccountDeleteDialog() {
        this.dialog
            .open(ConfirmModalComponent, {
                title: 'Delete Account',
                body: 'Your account will be deleted immediately and permanently. Once deleted, accounts can not be restored.',
                bodyBold: 'Do you want to delete the account?',
                ok: 'Delete',
            })
            .beforeClosed()
            .subscribe(confirmed => {
                if (confirmed) {
                    this.users
                        .delete([this.currentUser.get('id')], {deleteCurrentUser: true})
                        .subscribe(() => {
                            this.auth.logOut();
                        });
                }
